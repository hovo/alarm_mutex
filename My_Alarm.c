 ...
 * request expires. When the alarm request expires, it will
 * print another statement confirming its expiry.
 */
#include <pthread.h>
#include <unistd.h>
#include <stdio.h>
...
// Declare variables to handle the program if there are concurrent requests.
int requestCounter = 0;
int timeAccumulation;
...
void process_alarm(alarm_t *alarm) {
	...
	// Update expiry time to negate inconsistencies.
	 ...
	 * received, the alarm request itself, and its expiry time.
	 */
	...
	// Variable to compare against expiry time in the while loop.
	now = time(NULL);

	// Variable representing the number of seconds left in the print statement.
...
int main (int argc, char *argv[]) {
	...
	printf("Please enter an alarm request in the following format: # message\n");
	...
	printf("Example: 2 Good morning!\n");
	printf("Note: You can add successive alarm requests in the same format at any time during execution\n\n");

	// Handles the program's input, errors, and the main thread.
	while (1) {
		...
		if (sscanf(line, "%d %64[^\n]", &alarm->seconds, alarm->message) < 2 || alarm->seconds < 0) {
			fprintf(stderr, "Invalid alarm request. Please try again.\n");
			free(alarm);
		} else {
			if (requestCounter > 0) {
			...
			/*
			 * A3.2
			 * This segment of the code will let the user know that the
			 * main thread has received an alarm request along with the
			 * time it was received and the alarm request itself.
			 */
			printf("Main Thread Received Alarm Request at <%ld>: <%d %s>\n",
					time(NULL), alarm->seconds, alarm->message);

			// Increment the amount of alarm requests.
			requestCounter++;
